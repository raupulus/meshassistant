...
from functions import now_utc, now_utc_iso
import cron_tasks
...
class TestTracesSchedule(unittest.TestCase):
        ...
        with self.db._connect() as conn:
            conn.execute(
                ...
                (
                    ...
                    last_heard or int(now_utc().timestamp()),
    ...
    def test_client_reload_5_days_and_5_consecutive_errors(self):
        ...
        self._create_node(
            ...
            updated_at=initial_time.strftime("%Y-%m-%dT%H:%M:%SZ")
        )
        ...
        three_days_ago = (now - timedelta(days=3)).strftime("%Y-%m-%dT%H:%M:%SZ")
        with self.db._connect() as conn:
        ...
        six_days_ago = (now - timedelta(days=6)).strftime("%Y-%m-%dT%H:%M:%SZ")
        with self.db._connect() as conn:
        ...
        with self.db._connect() as conn:
            conn.execute("UPDATE nodes SET last_heard = ?, updated_at = ? WHERE node_id = ?",
                         (int(old_activity.timestamp()), old_activity.strftime("%Y-%m-%dT%H:%M:%SZ"), node_id))
            conn.commit()

        for i in range(5):
            t_err = self.db.enqueue_trace(node_id)
            self.db.mark_trace_done_with_route(t_err, False, text="err")
            err_date = (now - timedelta(hours=30 - i)).strftime("%Y-%m-%dT%H:%M:%SZ")
            with self.db._connect() as conn:
        ...
        with self.db._connect() as conn:
            conn.execute("UPDATE nodes SET last_heard = ?, updated_at = ? WHERE node_id = ?",
                         (new_last_heard, (now + timedelta(seconds=10)).strftime("%Y-%m-%dT%H:%M:%SZ"), node_id))
            conn.commit()
    ...
    def test_router_prioritized_at_06_am(self):
        ...
        with patch("Models.Database.now_madrid", return_value=datetime(2026, 8, 31, 6, 15, 0)):
    ...
    def test_send_trace_throttling_diurno_vs_nocturno_vs_router(self):
        ...
        self.db.mark_trace_done_with_route(t_id, True, text="ok")
        thirty_sec_ago = (now - timedelta(seconds=30)).strftime("%Y-%m-%dT%H:%M:%SZ")
        with self.db._connect() as conn:
        ...
        with patch("cron_tasks.env") as mock_env, patch("cron_tasks.Database", return_value=self.db):
            ...
            with patch("cron_tasks.now_madrid", return_value=datetime(2026, 8, 31, 6, 15, 0)), \
                 patch("Models.Database.now_madrid", return_value=datetime(2026, 8, 31, 6, 15, 0)):
                ...
                fifty_sec_ago = (now - timedelta(seconds=50)).strftime("%Y-%m-%dT%H:%M:%SZ")
                with self.db._connect() as conn:
