...
### ⚠️ La precisión se degrada con los días, y a partir del 5 el `periodo` viene VACÍO 🟢

**Verificado el 2026-08-26 con Vigo (36057).** Devuelve **7 días**, pero no todos con el mismo
detalle:

| Días | Troceado de las series |
|---|---|
| 1 y 2 | Tramos de **6 h** (`00-06`, `06-12`, `12-18`, `18-24`) además de los de 12 h y el `00-24` |
| 3 y 4 | Tramos de **12 h** (`00-12`, `12-24`) y el `00-24` |
| **5, 6 y 7** | **Un solo valor por serie, con `"periodo": ""`** |

Es decir, **el `periodo` vacío significa el día completo**. Si agrupas por ese campo sin normalizar,
los tres últimos días comparten la clave `""` y **se pierden o se machacan entre sí, sin ningún
error**: la respuesta es un `200` correcto con los 7 días dentro. Ver
[`ERRATAS.md` → A10](ERRATAS.md#a10-a-partir-del-día-5-la-predicción-diaria-devuelve-el-periodo-vacío-).

```php
$tramo = trim($periodo) === '' ? '0-24' : $periodo;
```

Otro detalle del mismo bloque: **la temperatura solo aparece en el tramo del día completo**. Si te
quedas con un tramo de 12 h esperando encontrarla ahí, los días 3 y 4 salen sin temperatura. Hay que
rellenarla desde el valor del día.
