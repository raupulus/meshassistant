...
### A10. A partir del día 5, la predicción diaria devuelve el `periodo` VACÍO 🟢

**Verificado el 2026-08-26** con `GET /api/prediccion/especifica/municipio/diaria/36057` (Vigo).

La predicción diaria devuelve 7 días. En los **cuatro primeros** cada serie viene troceada en tramos
identificados por `periodo`. A partir del **quinto día**, AEMET deja de trocear —el dato ya es del
día entero— pero **no omite la clave `periodo`: la deja como cadena vacía**:

```jsonc
// Día 3 — troceado en tramos de 6 h
{ "value": "35", "periodo": "12-18" }

// Día 5 — dato del día completo, pero el periodo viene vacío
{ "value": "20", "periodo": "" }
```

**Por qué rompe en silencio:** si agrupas por `periodo`, los días 5, 6 y 7 caen todos en una clave
`""`. Según cómo esté escrito el código, o se pierden esos tres días, o se machacan entre sí y solo
sobrevive el último. En nuestra primera implementación **solo se guardaron 4 de los 7 días** y no
saltó ningún error: la respuesta era un `200` correcto con los 7 días dentro.

**El `periodo` vacío significa "todo el día"**, es decir, equivale a `0-24`. Normalízalo antes de
agrupar:

```php
// Cadena vacía = día completo. Sin esto se pierden los días 5, 6 y 7.
$tramo = trim($periodo) === '' ? '0-24' : $periodo;
```

> Ojo, es **una tercera semántica** del mismo campo, que ya de por sí es ambiguo: 2 caracteres es una
> hora (`"12"` = las 12:00) y 4 caracteres es un tramo (`"0208"` = de 02:00 a 08:00).
> Ver [E8](#e8-periodo-tiene-dos-semánticas-en-el-mismo-objeto-).
...
Y hay una **tercera**: la cadena vacía, que significa "día completo".
Ver [A10](#a10-a-partir-del-día-5-la-predicción-diaria-devuelve-el-periodo-vacío-).
