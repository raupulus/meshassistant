...
| Método | Descripción |
|---|---|
| `get_node(node_id)` | Devuelve la fila como dict o `None` (incluye batería, voltaje y lecturas INA `power_ina1/2/3`). |
| `get_node_by_identifier(identifier)` | Busca un nodo por `node_id`, `short_name` o `name` con telemetría completa. |
| `get_router_nodes(configured_identifiers=None, max_hops=2, require_successful_trace_for_auto=True)` | Devuelve routers configurados siempre, y auto-detectados por rol (`ROUTER`/`ROUTER_LATE`/`REPEATER`) solo si han respondido con éxito un traceroute previo (incluye `power_ina1/2/3`). |
| `get_all_nodes()` | Devuelve el censo completo de nodos con todas sus columnas de señal y telemetría. |
| `create_node_if_not_exists(node_id, data=None)` | `INSERT OR IGNORE` + update opcional. |
| `update_node(node_id, data)` | Update con lista blanca de columnas (`role`, `hops`, `snr`, `battery`, `voltage`, `power_ina1`, `power_ina2`, `power_ina3`, etc.); castea `is_favorite`/`via_mqtt` a 0/1; actualiza `updated_at`. |
