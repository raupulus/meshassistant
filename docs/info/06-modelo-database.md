...
| Método | Descripción |
|---|---|
| `get_node(node_id)` | Devuelve la fila como dict o `None` (incluye batería, voltaje, saturación de canal `channel_util`, tiempo de transmisión `air_util_tx`, lecturas INA `power_ina1/2/3`, `is_watched` y `telemetry_count`). |
| `get_node_by_identifier(identifier)` | Busca un nodo por `node_id`, `short_name` o `name` con telemetría completa. |
| `get_router_nodes(configured_identifiers=None, max_hops=2, require_successful_trace_for_auto=True)` | Devuelve routers configurados siempre, y auto-detectados por rol (`ROUTER`/`ROUTER_LATE`/`REPEATER`) solo si han respondido con éxito un traceroute previo (incluye `power_ina1/2/3`, `channel_util`, `air_util_tx`, `telemetry_count`, `auto_report_count` y `auto_report_reason`). |
| `get_all_nodes()` | Devuelve el censo completo de nodos con todas sus columnas de señal y telemetría (batería, voltaje, canal, transmisión, INA, `is_watched`, `telemetry_count`, `auto_report_count` y `auto_report_reason`). |
| `create_node_if_not_exists(node_id, data=None)` | `INSERT OR IGNORE` + update opcional. |
| `update_node(node_id, data)` | Update con lista blanca de columnas (`role`, `hops`, `snr`, `battery`, `voltage`, `power_ina1`, `power_ina2`, `power_ina3`, `channel_util`, `air_util_tx`, `is_watched`, `telemetry_count`, `last_heard`, etc.); castea `is_favorite`/`is_watched`/`via_mqtt` a 0/1; actualiza `updated_at`. |
| `touch_node_last_heard(node_id, last_heard=None, short_name=None, name=None, hops=None, snr=None, rssi=None)` | Actualiza `last_heard` y `updated_at` de un nodo al recibir cualquier paquete o señal de radio (crea el nodo si no existe y asegura que `last_heard` nunca retroceda con `MAX`). |
| `set_node_watched(node_id, is_watched=True)` | Marca o desmarca un nodo para seguimiento en la pestaña Vigilancia (`is_watched = 1/0`). |
| `increment_node_telemetry_count(node_id)` | Incrementa en 1 de forma atómica el contador de paquetes de telemetría recibidos del nodo. |
