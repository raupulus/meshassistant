...
| `update_node(node_id, data)` | Update con lista blanca de columnas (`role`, `hops`, `snr`, `battery`, `voltage`, `power_ina1`, `power_ina2`, `power_ina3`, `channel_util`, `air_util_tx`, `is_watched`, `telemetry_count`, etc.); castea `is_favorite`/`is_watched`/`via_mqtt` a 0/1; actualiza `updated_at`. |
